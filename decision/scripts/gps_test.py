...
if __name__ == '__main__':
    ...
    gps_pub = rospy.Publisher('/uav2/mavros/global_position/set_gp_origin', GeoPointStamped, queue_size=10)
    home_pub = rospy.Publisher('/uav2/mavros/home_position/set', HomePosition, queue_size=10)
    rospy.wait_for_service('/uav2/mavros/cmd/set_home')
    home_client = rospy.ServiceProxy('/uav2/mavros/cmd/set_home', CommandHome)

    print("Service Created")
    rate = rospy.Rate(50)

    home_cmd = CommandHome()
    home_cmd.current_gps = False
    home_cmd.latitude = 41.1523373
    home_cmd.longitude = 116.2571147
    home_cmd.altitude = -9.5844799363
    
    # home_client1 = home_client(False,41.1523373,116.2571147,-9.5844799363)
    home_client1 = home_client(home_cmd.current_gps, home_cmd.latitude, home_cmd.longitude, home_cmd.altitude)
    print("Finish")
    rate.sleep()
    rospy.spin()
    # home_client1 = home_client([home_cmd.current_gps, home_cmd.latitude, home_cmd.longitude, home_cmd.altitude])
    # if home_client1.success:
    #     print("home client is successful")
    # drone_state = 10
    # drone_state_pub.publish(UInt64(drone_state))
    # px4 = Px4Controller()
    # px4.start()
    ...
    # rate = rospy.Rate(1)
    # gps = GeoPointStamped()
    # gps.position.latitude = 41.1523373
    # gps.position.longitude = 117.2571151
    # gps.position.altitude = -9.5844799363
    # home = HomePosition()
    # home.geo.latitude = 40.1523373
    # home.geo.longitude = 116.2571147
    # home.geo.altitude = -9.585480209
    # while True:
    #     gps_pub.publish(gps)
