...
class Px4Controller:
    ...
    def __init__(self):
        ...
        self.drone_state_pub = rospy.Publisher('drone_1/state', UInt64, queue_size=10)
    ...
    def start(self):
        ...
        while (rospy.is_shutdown() is False):
            ...
            self.cmd_vel = np.array([cmd_control[0], cmd_control[1], cmd_control[2]])
    ...
    def state_control(self, state):
        if state == self.task_takeoff:
            state_cmd_vel = self.pos_control(self.initial_pos, self.mav_pos, self.P_pos, self.pos_vel_sat)
            state_cmd_yaw = self.yaw_control(self.initial_pos, self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
            ...
            state_cmd_vel = self.pos_control(self.target_pos, self.mav_pos, self.P_pos, self.pos_vel_sat)
            state_cmd_yaw = self.yaw_control(self.target_pos, self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
    ...
    def dis_pos(self, desire_pos, feb_pos):
        ...
        dis_xy =  math.sqrt((desire_pos[0] - feb_pos[0]) * (desire_pos[0] - feb_pos[0]) + (desire_pos[1] - feb_pos[1]) * (desire_pos[1] - feb_pos[1]))
        return dis_xy
    ...
    def search(self):
        ...
        if self.dis_pos(self.search_pos[0], self.mav_pos) > 1 and self.search_point == 0:
            search_cmd_vel = self.pos_control(self.search_pos[0], self.mav_pos, self.P_pos, self.pos_vel_sat)
            search_cmd_yaw = self.yaw_control(self.search_pos[0], self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
            ...
            search_cmd_vel = self.pos_control(self.search_pos[1], self.mav_pos, self.P_pos, self.pos_vel_sat)
            search_cmd_yaw = self.yaw_control(self.search_pos[1], self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
        elif self.dis_pos(self.search_pos[1], self.mav_pos) < 1 and self.search_point == 1:
            search_cmd_vel = np.array([0,0,0])
            search_cmd_yaw = 0
            self.search_point = 2
        elif self.dis_pos(self.search_pos[2], self.mav_pos) > 1 and self.search_point == 2:
            search_cmd_vel = self.pos_control(self.search_pos[2], self.mav_pos, self.P_pos, self.pos_vel_sat)
            search_cmd_yaw = self.yaw_control(self.search_pos[2], self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
        elif self.dis_pos(self.search_pos[2], self.mav_pos) < 1 and self.search_point == 2:
            search_cmd_vel = np.array([0,0,0])
            search_cmd_yaw = 0
            self.search_point = 3
        elif self.dis_pos(self.search_pos[3], self.mav_pos) > 1 and self.search_point == 3:
            search_cmd_vel = self.pos_control(self.search_pos[3], self.mav_pos, self.P_pos, self.pos_vel_sat)
            search_cmd_yaw = self.yaw_control(self.search_pos[3], self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
        elif self.dis_pos(self.search_pos[3], self.mav_pos) < 1 and self.search_point == 3:
            search_cmd_vel = np.array([0,0,0])
            search_cmd_yaw = 0
            self.search_point = 4
        elif self.dis_pos(self.search_pos[4], self.mav_pos) > 1 and self.search_point == 4:
            search_cmd_vel = self.pos_control(self.search_pos[4], self.mav_pos, self.P_pos, self.pos_vel_sat)
            search_cmd_yaw = self.yaw_control(self.search_pos[4], self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
        elif self.dis_pos(self.search_pos[4], self.mav_pos) < 1 and self.search_point == 4:
            search_cmd_vel = np.array([0,0,0])
            search_cmd_yaw = 0
            self.search_point = 5
        elif self.dis_pos(self.search_pos[5], self.mav_pos) > 1 and self.search_point == 5:
            search_cmd_vel = self.pos_control(self.search_pos[5], self.mav_pos, self.P_pos, self.pos_vel_sat)
            search_cmd_yaw = self.yaw_control(self.search_pos[5], self.mav_pos, self.mav_yaw, self.P_yaw, self.yaw_sat)
        elif self.dis_pos(self.search_pos[5], self.mav_pos) < 1 and self.search_point == 5:
            search_cmd_vel = np.array([0,0,0])
            search_cmd_yaw = 0
            self.search_point = 6
